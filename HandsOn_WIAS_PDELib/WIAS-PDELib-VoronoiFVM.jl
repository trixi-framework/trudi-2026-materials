...
# ╔═╡ d970fb3a-e21f-436a-8741-8b973ef8ce52
using Triangulate: Triangulate, TriangulateIO, triangulate

# ╔═╡ 515fe902-50a6-466a-a50f-0b7364e8c3cb
using GridVisualize: plot_triangulateio

# ╔═╡ 4cdf7137-e8e3-4788-9d7c-92a94429fdd9
begin
    using ExtendableGrids: simplexgrid, bfacemask!
    using ExtendableGrids: Coordinates, CellNodes, CellRegions, BFaceNodes, BFaceRegions
end

# ╔═╡ 79d579af-db38-49f6-beae-08246b41b09a
begin
    using SciMLBase: solve
    using VoronoiFVM: VoronoiFVM, ramp, unknowns, fbernoulli_pm
    using VoronoiFVM: boundary_dirichlet!, boundary_neumann!
end

# ╔═╡ 1fb02d21-c674-44a3-9519-18b5947ffc9b
begin
    using SparseArrays: spzeros
    using ExtendableSparse: flush!, ExtendableSparseMatrixCSC
    using BenchmarkTools: @benchmark
end

# ╔═╡ 856d11e0-c16e-4064-b6c5-71d94f4c25f6
using OrdinaryDiffEqRosenbrock: ODEProblem, Rosenbrock23

# ╔═╡ 004f83ef-1e46-4a34-82ff-9a1bdcdb006a
using PlutoUI: PlutoUI, Resource
...
Trixi User and Developer Interaction (TRUDI) 2026
...
# ╔═╡ 5d21560e-3389-43cf-9d0d-1a232101eb3a
md"""
We present VoronoiFVM.jl, a Julia package for solving coupled nonlinear reaction-diffusion-convection PDE systems using two-point flux Voronoi finite volumes on admissible grids. We demonstrate the API for stationary and transient problems, multi-species systems, SciML solver integration, and drift-diffusion applications, highlighting automatic Jacobian assembly via ForwardDiff.jl.
"""

# ╔═╡ b1f7911d-3079-4094-b1cb-d2fe2c49e56a
md"""
## Problem class: Nonlinear Reaction-Diffusion-Convection
"""
...
- __Bold face__: ``n``-vectors, ``\vec{\text{arrows}}``: ``d``-vectors.
...
In vector form:
```math
\partial_t s(\mathbf u) - \nabla \cdot \mathbf{\vec{j}}(\mathbf u, \vec\nabla \mathbf u) + \mathbf r(\mathbf u) = \mathbf f
```

-  "Storage" ``\mathbf s: \mathbb{R}^n \to \mathbb{R}^n``
-  "Reaction" ``\mathbf r: \mathbb{R}^n \to \mathbb{R}^n``
-  "Flux" ``\mathbf{\vec{j}}: \mathbb{R}^n \times \mathbb{R}^{nd} \to \mathbb{R}^{nd}``
-  "Source" ``\mathbf f: \Omega\to \mathbb{R}^n``
-   ``\mathbf s, \mathbf{\vec{j}}, \mathbf r, \mathbf f`` can depend on ``\vec x, t`` as well.

"""


# ╔═╡ 40540ab6-f084-4902-9206-3698f8973983
md"""
## Boundary conditions
...
# ╔═╡ e93ba6f6-1f02-4d3b-bad7-7ff36c4ecb49
md"""
## Two point flux Voronoi finite volumes on admissible grids
"""

# ╔═╡ 9fe0696f-3955-4cfd-99ef-e608d6aadcb6
PlutoUI.ExperimentalLayout.hbox(
    [
        Resource(
            "https://www.wias-berlin.de/people/fuhrmann/blobs/fv2dpoly.png",
            :width => 300,
            :height => 225,
        ),
        ...
        md"""
         -  ``\Omega`` ``|>`` triangulation ``|>``	__Voronoi cells__ ``\equiv`` control volumes
        ```math
            \int_{\omega_k} \nabla \cdot \vec{\mathbf j}\, d\vec x= \int_{\partial\omega_k} \vec{\mathbf j} \cdot \vec \nu\,ds
        ```
        -  Approximate normal flux ``\mathbf{\vec j}\cdot \vec \nu_{kl}`` between neigboring control volumes ``\omega_k,\omega_l`` by finite difference expression ``\mathbf g\left(\mathbf u(\vec x_k) , \mathbf u(\vec x_l)\right)``
        - This requires __admissible__ meshes: ``\sigma_{kl} \perp \overline{\vec x_k\vec x_l}`` """,
    ]
)


# ╔═╡ 76ca43a9-54e2-499d-a34e-1108f3d43f96
md"""
- Time subdivision: ``0=t^0 < t^1 < \dots < t^{M-1}< t^M=T``
- Let ``\mathbf u_k^i= \mathbf u(\vec x_k, t^i)``
- Implicit Euler two point flux scheme -- for ``k=1\dots N``:

```math
|\omega_k|\frac{\mathbf s(\mathbf u_k^m)-\mathbf s(\mathbf u^{m-1}_k)}{t^m-t^{m-1}} + \displaystyle\sum_{l\in\mathcal N_k} \frac{|\sigma_{kl}|}{h_{kl}}\mathbf g(\mathbf u_k^m , \mathbf u_l^m) + |\omega_k|\mathbf r(\mathbf u_k^m) + |\gamma_k|\mathbf a(\mathbf u_k^m) = |\omega_k| \mathbf f_k + |\gamma_k|\mathbf b_k
```
- ``\vec x_k``: collocation point, ``\omega_k``: control volume
...
# ╔═╡ d7704c8c-a621-42d3-a231-1463fd8134a0
md"""
## Towards an API
"""

# ╔═╡ 18bb7b83-ab22-4561-865a-4031428d47e6
md"""
As a consequence, the finite volume scheme can be described by the discretization __grid__
giving rise to the geometry derived data ``\omega_k``,``\gamma_k``, ``\sigma_{kl}``, ``h_{kl}``
and the __physics__ function
- `flux`  ``\mathbf g: \mathbb{R}^n \times \mathbb{R}^n \to \mathbb{R}^n``
- `storage` ``\mathbf s: \mathbb{R}^n \to \mathbb{R}^n``
- `reaction` ``\mathbf r: \mathbb{R}^n \to \mathbb{R}^n``
- `source` ``\mathbf f: \Omega\to \mathbb{R}^n``
- `breaction` ``\mathbf a: \mathbb{R}^n \to \mathbb{R}^n``
- `bsource`   ``\mathbf b: \partial\Omega\to \mathbb{R}^n``
"""

# ╔═╡ 07ac3412-e34d-48d2-95c5-e9bd9e77caeb
md"""
## How to create the Voronoi cells ?
"""

# ╔═╡ 7abfd13d-c696-4381-97e4-88c014351fe8
md"""
Start with a Delaunay triangulations. As a demonstration, we show how to use `Triangulate.jl`, a Julia wrapper of J. R. Shewchuk's `Triangle` code to create a Voronoi tesselation.  It is created from a boundary conforming Delaunay grid (warranted by the "D"-flag in the control string).
"""

# ╔═╡ 5e42cdfc-f094-4fc9-ba94-7e9b46e6d6ae
import CairoMakie

# ╔═╡ 874667a1-63fd-4b7a-b7ee-dc63677a0600
begin
    using GridVisualize: GridVisualize, GridVisualizer, scalarplot, scalarplot!, gridplot, reveal
    GridVisualize.default_plotter!(CairoMakie)
end;

# ╔═╡ 259a117d-9346-4386-af31-2742baba6a7f
begin
    triin = TriangulateIO()
    ...
    triin.regionlist = Matrix{Cdouble}([0.2 0.8; 0.2 0.2; 1 3; 0.02 0.05])
    triin
end

# ╔═╡ 97feb82c-c7d2-48f9-b021-0746f53d6878
triout, vorout = triangulate("paADq20Qv", triin)

# ╔═╡ cf4861ea-6b32-4844-a7c7-b33ed8f883ee
md"""
## Visualization of triangles and Voronoi cells
"""

# ╔═╡ df4a9826-8f0d-44b9-9f31-ce32624303d9
plot_triangulateio(triin, triout; voronoi = vorout, Plotter = CairoMakie, size = (400, 400))

# ╔═╡ 52445021-00fb-4cad-9bde-09610f64f7d5
md"""
## Create computational grid
"""

# ╔═╡ 0a31b15d-a74c-4582-a573-08fcfc3fb726
grid = simplexgrid(triout);

# ╔═╡ ad4d2af1-8b7e-486d-bfe4-75687d2f9651
gridplot(grid, size = (400, 400))

# ╔═╡ 9984e2ae-80b3-45d1-9e51-f2831f383e6b
md"""
## 1D scalar stationary reaction-diffusion
"""

# ╔═╡ 0de2eb91-f22b-48a6-8c6a-944622ebf29e
md"""
Let ``Ω=(0,1)``, ``Γ_1=\{0\}``, ``Γ_2=\{1\}``, solve
```math
\begin{aligned}
- ∇\cdot (D\nabla u) + Ru^2&= 0\\
				u|_{Γ_1}&=1\\
...
# ╔═╡ 26d8f471-2418-4e6e-a983-d56e294bd238
md"""
## Reaction Diffusion Physics
"""

# ╔═╡ 1ae405a9-e6fb-4ed3-b6ee-2c6fd482ccb6
function rdflux(y, u, edge, data)
    y[1] = data.D * (u[1, 1] - u[1, 2])
    return nothing
end;

# ╔═╡ d5c9c9b8-e355-4910-8ed2-e6164ce45365
function rdreaction(y, u, node, data)
    return y[1] = data.R * u[1]^2
    nothing
end;

# ╔═╡ a80fe5f4-bf94-4599-bf10-02242c9a6dc5
function rdbc(y, u, bnode, data)
    boundary_dirichlet!(y, u, bnode, region = 1, value = 1, species = 1)
    boundary_dirichlet!(y, u, bnode, region = 2, value = 0, species = 1)
    boundary_neumann!(y, u, bnode, region = 3, value = 0)
    return nothing
end;

# ╔═╡ f9a4f8a5-3367-4944-a501-083500b26e68
function rdstorage(y, u, node, data)
    y .= u
    return nothing
end

# ╔═╡ 8f7144df-e8d2-4cb6-b1ce-c484e553d969
rdphysics = VoronoiFVM.Physics(storage = rdstorage, flux = rdflux, reaction = rdreaction, breaction = rdbc)

# ╔═╡ b5d66728-bfd8-437e-ae62-d9dc841b0857
md"""
## 1D Reaction Diffusion system and solution
"""

# ╔═╡ 7532d5da-264d-49dc-a869-804f20c8600e
rddata = (D = 2, R = 100);

# ╔═╡ bc5420d1-4b3c-4a90-b081-92f70f38f41f
rd1dgrid = simplexgrid(0:0.01:1);

# ╔═╡ 45be9b74-c047-4389-89d5-609719a33e83
rd1dsystem = VoronoiFVM.System(rd1dgrid, rdphysics; species = [1]);

# ╔═╡ 4df6d03a-2dea-4994-8fcd-daa180c47119
rd1dsol = solve(rd1dsystem; data = rddata);

# ╔═╡ 80ae99cd-9a06-492f-8156-baf9ec3739f7
scalarplot(rd1dgrid, rd1dsol[1, :], size = (700, 200))

# ╔═╡ 44ff47a6-90d0-4bb5-a6d6-bf127952fbdb
md"""
## 2D scalar stationary reaction-diffusion
"""

# ╔═╡ 01784c8c-e5e1-4742-898d-47d1e428c8a8
md"""
Let ``Ω=(0,1)``, ``Γ_1=\{0\}×(0,0.1)``, ``Γ_2=\{1\}×(0,0.1)``,
``Γ_3=∂ Ω∖(Γ_1∪Γ_2)`` solve
```math
\begin{aligned}
- ∇\cdot (D\nabla u) + Ru^2&= 0, 
...
# ╔═╡ e2f8502d-06a2-4be4-a13e-ab14e1289a9f
begin
    rd2dgrid = simplexgrid(0:0.01:1, 0:0.01:0.1)
    bfacemask!(rd2dgrid, [0, 0], [1, 1], 3, allow_new = false)
    bfacemask!(rd2dgrid, [0, 0], [0, 1], 1)
    bfacemask!(rd2dgrid, [1, 0], [1, 1], 2)
end;

# ╔═╡ dc10f4e7-b4e3-4fde-9737-230731260210
gridplot(rd2dgrid, linewidth = 0.1, size = (700, 200))

# ╔═╡ 74073959-df42-46d7-896e-68bc78c40cc8
rd2dsystem = VoronoiFVM.System(rd2dgrid, rdphysics; species = [1]);

# ╔═╡ 377b8314-2287-4592-a957-99fe96ca09ae
rd2dsol = solve(rd2dsystem; data = rddata);

# ╔═╡ 3bd68084-1579-443a-846c-ba0afd17e69b
scalarplot(rd2dgrid, rd2dsol[1, :], size = (600, 200))

# ╔═╡ 9ef60d6d-bffc-4689-8ce6-7584c813a0eb
md"""
## What is going on here ?
"""

# ╔═╡ 322e2ae5-d88e-455d-a759-0af73fbb12c7
md"""
...
- Calculation of the data derived from the geometry: ``|\omega_k|``, ``|\frac{\sigma_{kl}}{h_{kl}}|``, ``|\gamma_k|``. This then allows to piece together a nonlinear operator ``A: \mathbf R^N \to \mathbf R^N`` essentially defined on the graph made of the edges and nodes of the triangulation, where ``A(u)=0`` gives the solution of the discrete problem.
- Iterative solution by (damped) Newton method: given ``u_m``, iterate until ``||d_m||<`` tol:
```math
	\begin{aligned}
    \text{residual:}\;     r_m&= A(u_m)\\
    \text{solve}\; A'(u_m)d_m&= r_m\\ 
    \text{update:}\;  u_{m+1} &= u_m-d_m
    \end{aligned}
```
"""

# ╔═╡ fae2d5d9-0e95-4203-bdeb-8bbadb869723
md"""
...
- The values are assembled into the residual vector ``r_m``, and the local Jacobi matrices are assembled into the global Jacobi matrix ``A'(u_m)``.   Efficient sparse matrix assembly is supported by `ExtendableSparse.jl`
- The linear system yielding the Newton update is solved using sparse direct or iterative solvers available via `LinearSolve.jl`
"""

# ╔═╡ 2b54aa96-b17d-4b1c-b88b-c4792ffb4381
md"""
## ExtendableSparse.jl

- `ExtendableSparseMatrixCSC<:AbstractSparseMatrixCSC` wraps a linked list sparse matrix structure for efficient insertion and a `SparseMatrixCSC`
- `flush!` method efficiently collects new elements into the `SparseMatrixCSC`
- Works with sparse linear algebra methods which take an `AbstractSparseMatrixCSC`
- Recently updated for multithreading
"""

# ╔═╡ a5757ba0-86b3-4383-a29a-7fbb6a329884
function create_sparse_matrix(A)
    n = size(A, 1)
    for i in 1:n
        A[i, i] = 100
        i > 1 ? A[i, i - 1] -= 10 : nothing
        i < n ? A[i, i + 1] -= 10 : nothing
    end
    return flush!(A)
end;

# ╔═╡ 3ab8d961-556c-460d-ad54-1079bdead8a8
N = 10000;

# ╔═╡ c8924f5a-b4a5-4b2f-a6d2-74c309428c82
@benchmark create_sparse_matrix(spzeros(N, N))

# ╔═╡ 46908e89-90c7-4ab0-905a-3e22179c8c68
@benchmark create_sparse_matrix(ExtendableSparseMatrixCSC(N, N))

# ╔═╡ 18357cd8-7a34-431a-9420-8aec7a247d30
create_sparse_matrix(spzeros(N, N)) * ones(N) ≈ create_sparse_matrix(ExtendableSparseMatrixCSC(N, N)) * ones(N)

# ╔═╡ d5d7226e-63b2-4ef4-aa95-78ca71c6d54e
## Two-species reaction-diffusion system

# ╔═╡ c2a2ebd5-fefa-488e-b62b-9e871e644b9b
md"""
## Transient system solution
...
\begin{aligned}
	...
	\partial_t  u_2  - \nabla \cdot D_2 \nabla u_2 - R(u_1, u_2) & =0\\
                          R(u1, u2) &= k^+u_1 - k^- u_2^2\\
						 u_1|_{t=0}=u_2|_{t=0} &=0\\
							u_1|_{Γ_1} & =1, &
	                        D_1\partial_n u_1|_{Γ_2} &=0\\
                            D_2\partial_n u_2|_{Γ_1} &=0&
						    u_2|_{Γ_2} & =0
\end{aligned}
```
...
# ╔═╡ df81b15f-f2e3-42cb-a374-608fbfc7a569
function rdsflux(y, u, edge, data)
    (; D₁, D₂) = data
    y[1] = D₁ * (u[1, 1] - u[1, 2])
    y[2] = D₂ * (u[2, 1] - u[2, 2])
    return nothing
...
# ╔═╡ 7780dc1f-dff7-433e-b0e8-23dde219ed48
function rdsreaction(y, u, node, data)
    (; k⁺, k⁻) = data
    R = k⁺ * u[1] - k⁻ * u[2]^2
    y[1] = R
    y[2] = -R
...
# ╔═╡ d795feee-122a-444b-94b9-274e8023724b
function rdsstorage(y, u, node, data)
    y[1] = u[1]
    y[2] = u[2]
    return nothing
end;

# ╔═╡ 2c5de675-a3ec-4767-b3a1-a19ac3b18c0d
function rdsbc(y, u, bnode, data)
    boundary_dirichlet!(y, u, bnode, species = 1, value = 1, region = 1)
    boundary_dirichlet!(y, u, bnode, species = 2, value = 0, region = 2)
    return nothing
end;

# ╔═╡ d7597bb8-4ad5-4367-89f8-33bd8e094079
rds1ddata = (k⁺ = 1.0, k⁻ = 0.1, D₁ = 1, D₂ = 0.1);

# ╔═╡ 45a03014-69a2-49b5-aafd-8e1bfd96261a
rdsphysics = VoronoiFVM.Physics(storage = rdsstorage, flux = rdsflux, reaction = rdsreaction, breaction = rdsbc, data = rds1ddata);

# ╔═╡ 7d8b67c6-76d3-46ae-848c-6149997cc312
rds1dsys = VoronoiFVM.System(rd1dgrid, rdsphysics; species = [1, 2]);

# ╔═╡ 2edaf906-e328-4fec-91b9-d8a3b6f54eac
rds1dinival = unknowns(rds1dsys, inival = 0);

# ╔═╡ d6ee757c-087b-4c9c-9bbf-52195a2bea95
tend = 100.0;

# ╔═╡ c1dd1e81-8d96-4ac0-8124-42e5053e89ec
rds1dtsol = solve(rds1dsys; inival = rds1dinival, times = (0, tend), Δt = 1.0e-4, force_first_step = true)

# ╔═╡ 84aa0c8c-c2dd-402f-9b5c-1fe90c1b7205
md"""
## Visualizing time dependent solution
"""

# ╔═╡ 6a22a36a-2534-4c81-a595-db3e5ba7aa86
typeof(rds1dtsol) |> supertype |> supertype

# ╔═╡ 3b8c5bbc-685d-4d99-b030-febcb95146cf
md"""
- `rd1dtsol[k,j,k]` is the value of the solution for species `i` in point `j` and timestep `k`.
- `rd1dtsol(t)` interpolates the solution for time `t` and returns an `nspecies`×`npoints` matrix
"""

# ╔═╡ ca824f13-65a7-4b51-86e2-7207911de242
@bind rds1dt PlutoUI.Slider(range(0, rds1dtsol.t[end], length = 101), show_value = true)

# ╔═╡ 2a1ae93d-b330-4118-aada-bcdf50d231d2
let
    u = rds1dtsol(rds1dt)
    vis = GridVisualizer(size = (700, 200), limits = (-0.1, 4), legend = :rt)
    scalarplot!(vis, rd1dgrid, u[1, :], color = :red, label = "u₁")
    scalarplot!(vis, rd1dgrid, u[2, :], color = :green, label = "u₂", clear = false)
    reveal(vis)
end

# ╔═╡ efc9894d-edf7-400d-8ae5-bd4111eeb413
md"""
## Using differential equation solvers from SciML
"""

# ╔═╡ 8bd843dd-9a41-4255-bf87-788dff7975aa
problem = ODEProblem(rds1dsys, rds1dinival, (0, tend));

# ╔═╡ 316710d4-f65e-4a21-b878-6c0d92fc7554
odesol = solve(problem, Rosenbrock23(), dt = 1.0e-4, reltol = 1.0e-4);

# ╔═╡ 38616dcd-68bc-4db2-beff-62356db7f8c6
rds1dodesol = reshape(odesol, rds1dsys);

# ╔═╡ 609cda4e-c3da-4fa8-b12a-5d5502c72cb4
@bind rds1dodet PlutoUI.Slider(range(0, rds1dodesol.t[end], length = 1001), show_value = true)

# ╔═╡ d36aab9d-0336-48eb-bb9a-36bca84d6683
let
    u = rds1dodesol(rds1dodet)
    vis = GridVisualizer(size = (700, 200), limits = (-0.1, 4), legend = :rt)
    scalarplot!(vis, rd1dgrid, u[1, :], color = :red, label = "u₁")
    scalarplot!(vis, rd1dgrid, u[2, :], color = :green, label = "u₂", clear = false)
    reveal(vis)
...
# ╔═╡ 05c24926-173c-4cbf-8bc9-9d3b099d0920
md"""
## Unipolar Drift-Diffusion
"""

# ╔═╡ 30d5bcaa-6029-4395-a102-e361d66f5fcd
md"""
See: C. Cancès, C. Chainais-Hillairet, J. Fuhrmann, and B. Gaudeul, "A numerical-analysis-focused comparison of several finite volume schemes for a unipolar degenerate drift-diffusion model" [IMA J. Numer. Analysis, 41(2021)1, 271–314](https://doi.org/10.1093/imanum/draa002), [arxiv1907.11126](https://arxiv.org/abs/1907.11126).
...
-\nabla \varepsilon \nabla \phi = z(2c-1)
```
Degenerate drift-diffusion equation for transport of a charged species $c$:

```math
\partial_t u  - \nabla\cdot \left(\nabla c  + c \nabla (\phi - \log (1-c) )\right)
```

In particular, the paper, among others, investigates a flux discretization which is able to handle the degeneracy coming from the $\log (1-c)$ term. The earliest reference to this scheme we found in the source code of the [SEDAN III](http://www-tcad.stanford.edu/tcad/programs/sedan3.html) semiconductor device simulator. An important feature of this model is the boundedness of the species concentration ``0<c<1``.
"""

# ╔═╡ 9147c449-d085-4eca-90b9-3552bbd83a11
Base.@kwdef mutable struct UDDData
    eps::Float64 = 1.0e-3
    z::Float64 = -1
    ic::Int32 = 2
    iphi::Int32 = 1
...
# ╔═╡ a572155d-d7dd-44d2-a502-283c55720cce
function uddstorage!(f, u, node, data)
    (; ic, iphi) = data
    f[iphi] = 0
    f[ic] = u[ic]
    return
...
# ╔═╡ e9c0d6cf-dc88-4218-9c65-1f25d1aed141
function uddreaction!(f, u, node, data)
    (; ic, iphi) = data
...
# ╔═╡ d021f3e8-1cf1-41d9-b0b9-fd2515a1c7ea
function sedanflux!(f, u, edge, data)
    (; ic, iphi, z) = data
    f[iphi] = data.eps * (u[iphi, 1] - u[iphi, 2])
    mu1 = -log1p(max(-1, -u[ic, 1]))
    mu2 = -log1p(max(-1, -u[ic, 2]))
    bp, bm = fbernoulli_pm(z * 2 * (u[iphi, 1] - u[iphi, 2]) + (mu1 - mu2))
    f[ic] = bm * u[ic, 1] - bp * u[ic, 2]
    return
end;

# ╔═╡ f3ff32fb-81cb-462a-aeeb-d89f4c766596
@doc fbernoulli_pm

# ╔═╡ 3ab980c8-d413-4866-93ba-e151e8953f14
function uddbc!(f, u, bnode, data)
...
# ╔═╡ a447e22f-ea2a-4aec-8457-3f974279d3fd
udddata = UDDData();

# ╔═╡ 78f48e4e-8c0d-4934-ab13-f1b5d9217af5
uddgrid = simplexgrid(range(0, 1, length = 51));

# ╔═╡ 8316cf40-a4ce-4eef-8f2d-4bf9dd6a0084
uddphysics = VoronoiFVM.Physics(;
    data = udddata,
    flux = sedanflux!,
    reaction = uddreaction!,
    breaction = uddbc!,
    storage = uddstorage!
)

# ╔═╡ 57b9dcf4-7555-477a-8bfb-eef22567700b
uddsys = VoronoiFVM.System(uddgrid, uddphysics; species = [udddata.iphi, udddata.ic]);

# ╔═╡ 84db7ecd-38eb-4d3d-8f11-caf14bfe6a0e
begin
    uddinival = unknowns(uddsys)
    uddinival[udddata.iphi, :] .= 0
    uddinival[udddata.ic, :] .= 0.5
end;

# ╔═╡ 53ebe294-0f81-4873-9731-37f8a8eb4478
begin
    tstep = 1.0e-5
    ...
    control.Δt_max = 0.1
    control.Δu_opt = 0.1
    control.damp_initial = 0.75
end

# ╔═╡ 3a09d130-b16d-483b-a41a-c4f490e23a10
uddtsol = solve(uddsys; inival = uddinival, times = [0.0, 10], control, verbose = "ne")

# ╔═╡ 275cb89b-a5f2-40ce-b725-c7ee885b3e40
@bind uddt PlutoUI.Slider(logrange(1.0e-5, uddtsol.t[end], 101), show_value = true)

# ╔═╡ c7832513-84fc-4656-832c-3214a212fd11
let
    vis = GridVisualizer(size = (700, 200), legend = :rt)
    sol = uddtsol(uddt)
    scalarplot!(
        vis,
        uddgrid,
...
# ╔═╡ c88613b6-d0d2-4b54-ba7d-c0accb1371d6
md"""
## More features

- Extended multiphysics: different fluxes in different regions, boundary species
- Automatic Jacobian assembly via [ForwardDiff.jl](https://github.com/JuliaDiff/ForwardDiff.jl)
- [SciML](https://sciml.ai/) integration: use solvers from [OrdinaryDiffEq.jl](https://github.com/SciML/OrdinaryDiffEq.jl) for time stepping
- Flexible linear solver selection via [LinearSolve.jl](https://github.com/SciML/LinearSolve.jl)
- Efficient sparse matrix assembly via [ExtendableSparse.jl](https://github.com/WIAS-PDELib/ExtendableSparse.jl) with multithreading support
- Parameter sensitivity: derivatives of solutions w.r.t. parameters
- Solution vectors can be dense or sparse matrices
- Capable of general `eltype` for solution vectors ⇒ autodiff, MultiFloats
- Small signal analysis in frequency domain ⇒ Impedance spectroscopy
...
- [MultiComponentReactiveMixtureProject](https://github.com/DavidBrust/MultiComponentReactiveMixtureProject): Heat and multi-component reactive gas transport in porous media
- [MosLab.jl](https://github.com/Rapos0/MOSLab.jl): From semiconductor to transistor level modeling
- Redox flow cells, mixture flows, two-phase geothermal flows
"""

# ╔═╡ e533e538-d73f-4f13-9929-6a21fbe5be79
...
OrdinaryDiffEqRosenbrock = "~1.25.0"
PlutoUI = "~0.7.79"
SciMLBase = "~2.149.0"
Triangulate = "~3.0.1"
VoronoiFVM = "~3.2.0"
...
manifest_format = "2.0"
project_hash = "d027fc9020decc91831c9c05bd5a38919aa6c934"
